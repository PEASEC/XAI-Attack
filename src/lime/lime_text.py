...
class LimeTextExplainer(object):
    ...
    def __data_labels_distances(self,
        ...
        if is_sentence_pair_task:
            inverse_data_seperated = [x.split("_SEP_") for x in inverse_data]
            inverse_data_seperated = list(map(list, zip(*inverse_data_seperated)))
            labels = classifier_fn((inverse_data_seperated[0], inverse_data_seperated[1]))
        else:
